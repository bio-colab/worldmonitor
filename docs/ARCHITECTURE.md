...
### Dependency Bootstrap Boundary (Install-Time Performance)

To avoid unnecessary install-time cost, dependency bootstrapping is split by responsibility:

- Root `npm install` only installs core dashboard dependencies.
- Optional sub-app dependencies (for example `blog-site` and `pro-test`) are installed **on-demand** via `scripts/bootstrap-deps.mjs` when their build targets run.
- A compatibility escape hatch (`WM_INSTALL_OPTIONAL_WORKSPACES=1`) is supported for environments that still want eager postinstall behavior.

This keeps default local setup fast while preserving an explicit extension point for future optional workspaces without coupling them to every install.
